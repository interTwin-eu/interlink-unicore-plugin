...
# TODO: make it configurable while executing the script
...
global base_url
base_url = InterLinkConfigInst["UnicoreSiteUrl"]
...
def produce_unicore_apptainer_script(containers, metadata, commands, input_files):
    ...
    for c in containers:
        if "image" in c.keys():
            #user_precommand += str("apptainer pull image"+str(img_count)+".sif "+c["image"]+"; ")
            #img_count += 1
            user_precommand += str("apptainer pull "+c["name"]+".sif "+c["image"]+"; ")
    ...
    try:
        ...
        print("Main command: ",exec_command)
        print("Pre command: ",user_precommand)
